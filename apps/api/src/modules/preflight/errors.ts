...
const PATTERNS: ErrorPattern[] = [
  ...
  {
    pattern: /parse.*invoice|invalid invoice|invalid.*bech32|bech32|checksum|malformed/i,
    category: 'invoice',
  ...
  {
    pattern: /allow_self_payment|pay to self|self.?payment|can ?not pay to self/i,
    category: 'routing',
    title: 'Cannot pay your own invoice',
    explanation:
      'This invoice was generated by the same node that is trying to pay it. Fiber blocks self-payment unless allow_self_payment is enabled — and a real metering flow should pay a separate payee node anyway.',
    suggestions: [
      'Use two nodes: have a separate payee node issue the invoice, then pay it from your node (open a channel between them first).',
      'For a quick single-node smoke test only, enable allow_self_payment in your Fiber node config and restart.',
    ],
  },
  {
    pattern:
      /no.*channel|channel.*not.*found|no active channel|no available channel|not enough channels?|no funded channel|channels?.*not ready|no.*ready.*channel/i,
    category: 'routing',
    title: 'No usable channel',
    explanation:
      'Your node has no open, ready channel that can reach the payee, so it cannot build a payment route.',
    suggestions: [
      'Open a channel to the payee (or a well-connected peer) with open_channel, then wait for ChannelReady.',
      'Fund the channel with enough outbound liquidity to cover the invoice amount plus fees.',
      'Confirm the channel asset matches the invoice (CKB vs a UDT).',
    ],
  },
  {
    pattern:
      /no route|route not found|failed to (find|build).*(path|route)|no path|cannot find.*(path|route)|unreachable/i,
    category: 'routing',
