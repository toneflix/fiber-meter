...
import { calculateCharge, Decimal, money } from '../../utils/money.js'
  ...
  async record(
    ...
    if (existing) {
      return {
        ...
        amount: money(existing.calculatedAmount),
    ...
    return prisma.$transaction(async (tx) => {
      const service = await tx.meteredService.findUnique({
        where: { developerId_slug: { developerId, slug: input.service } },
      })
      ...
      const customer = await tx.customer.findUnique({
        where: { developerId_externalId: { developerId, externalId: input.customer } },
      })
      ...
      const amount = calculateCharge(rule.pricingModel, rule.price, input.quantity)
      const balance = await tx.balance.findUnique({
        where: { customerId_asset: { customerId: customer.id, asset: rule.asset } },
      })
      const available = new Decimal(balance?.availableBalance ?? 0)
      ...
      return {
        ...
        amount: money(amount),
        asset: rule.asset,
        balanceRemaining: money(updatedBalance.availableBalance),
      }
    })
  }
}
